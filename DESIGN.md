...
current_design_stage: "PUSH-033：单设备学生会话、OCR 跨刷新入口、苏格拉底导师与任务型错题本已接入；计划、进步、事实报告和全路径生产验收继续推进"
...
**已新增**：匿名设备会话在首次进入时准备学生空间，Today 与上传页可显示当前设备下未完成的 OCR 批次，并按“继续添加/等待/核对/重试”进入真实下一步；不伪造旧图片预览。guided task 内提供“把你的思路说出来”苏格拉底面板，每轮只显示一个问题和可选提示。一级导航新增任务型错题本，顶部只保留当前位置和真实可点击的“学习设置”，桌面/移动端一致；全局错误页与 404 均提供可执行返回路径。

**尚未完成**：三题检查仍是无记录的原创合成体验题；当前匿名身份不支持跨设备账号同步。错题本展示正式任务记录，不是逐题 OCR 原图档案。计划、进步、事实报告仍为基于当前事实的空状态；生产 OSS、原图确认后 24h 删除/主动删除、完整键盘/移动端与真实学生可用性验收尚未完成。任何 OCR、导师或复测工程结果均不能称真实学习效果。

**下一步**：实现真实计划、进步和事实报告投影，并补逐题 OCR 错题档案契约；随后对全部可点击入口、失败态、键盘、移动端、跨刷新和真实 Provider 做集中验收。任何新字段不得从视觉稿或合成 Fixture 反推业务状态。
