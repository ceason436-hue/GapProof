...
current_design_stage: "PUSH-079 已发布：我的材料支持服务端搜索、状态筛选与游标继续加载；比赛功能落地优先，测试与审查 deferred"
...
**下一步**：比赛提交前优先继续补齐学生端真实可操作功能；OCR 题目分段/人工拆题、完整导师历史和生产 OSS 删除按可演示价值推进。全部可点击入口、失败态、键盘、移动端、跨刷新和真实 Provider 的集中验收已记录为 deferred，不在当前功能实现前置阻塞。任何新字段不得从视觉稿或合成 Fixture 反推业务状态。
...
- “我的材料”标题区下增加名称搜索和“全部 / 待继续 / 已有记录”筛选，复用错题本紧凑工具栏；结果摘要同时显示当前数量、筛选命中数与材料总数。
- 首批显示 20 份，存在更早材料时显示“继续显示更多材料”；加载失败保留已有卡片和搜索条件，提供明确重试。手机端搜索与三个筛选按钮纵向适配。
- “已有记录”只表示同一材料当前存在终态任务事实，不等于永久掌握或学习效果；本批视觉与浏览器验收 deferred。
...
- 错题本默认只展开最近 20 道，学生可用“继续显示”每次展开最多 20 道；摘要同时说明当前展示数、筛选命中数和错题本总数。
- 搜索、状态筛选或清空条件后回到首批，避免筛选结果沿用旧的展开长度；无结果时继续提供清空条件动作。
- 每道题的“查看题目”继续进入相同详情页，但详情改为只读取当前题目的受保护事实，不在页面间传递整本记录。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。
...
- 已完成的针对练习在任务回顾页显示当时的导师对话，可按需展开当时提示，但没有输入框或继续提问按钮。
- 每轮继续区分学生思路、导师问题和备用引导；没有保存对话时明确说明，不用示例内容填充。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.76 — 2026-08-16

- 已完成的明日复习和 7 天后巩固在任务回顾页显示“你当时选择”、本次记录和完成时间，不再只有原题与选项。
- 结果使用“通过、需要继续巩固、需要老师或家长协助”的学生语言；不显示答案、选择 ID 或内部评分信息。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.75 — 2026-08-16

- 从 7 日计划或错题本打开旧任务时，任务页直接读取该任务本身，不会因为它不在 Today 当前列表中而显示“没有找到”。
- 等待任务继续只读查看安排，完成任务继续只读回顾，可开始任务才显示既有作答或引导控件。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.74 — 2026-08-16

- 首次学生从 Today、侧栏“找原因”、三题链接或上传链接进入时，均先看到同一品牌化学习范围选择，不会被直接带到与自己设置无关的检查页。
- 五项内容继续全部由学生确认，未选择时不显示默认年级、学科、学期、地区或学习状态。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.73 — 2026-08-16

- 没有上传材料和正式 Case 的首次学生完成三题后，返回 Today 也会看到完成提示，不再重新回到没有反馈的起点。
- 提示继续提供上传自己的错题和重新做三题两项动作，并明确不进入正式学习记录、报告或掌握度。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.72 — 2026-08-16

- Today“稍后继续”的复习卡不再只有“可以开始”时可点击；等待中的任务可查看安排，完成后的任务可回顾记录。
- 三种动作均进入同一学生任务页，页面继续明确区分等待、可作答和只读回顾状态。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.71 — 2026-08-16

- 针对练习、明日复习、7 天后巩固和错题重做统一进入“学生任务”页面，不再因历史路由让所有任务看起来都属于错题详情。
- 普通任务页返回 Today，错题重做返回错题本；旧任务链接继续可用并自动进入新页面。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。
...
- 错题较多时可按题干或材料来源搜索，并切换全部、可继续/待复习、已完成；没有结果时可一键清空条件。
- 报告列表的标题和主动作进入单份详情，详情提供返回报告列表、错题本、我的进步和今日任务的真实链接；示例来源继续明确披露。
- 按比赛功能落地优先要求，本批测试、视觉、浏览器及真实学生验收继续 deferred。

### v0.2.68 — 2026-08-16

- 恢复已有多图材料时直接显示页面编号和学生可理解的处理状态，可继续添加、逐页替换或移除；不伪造已经上传图片的预览。
- 首次选择后继续使用紧凑“继续添加”，手机端页面缩略块与文字保持两列，操作按钮另起一行，避免队列挤压溢出。
- 按比赛功能落地优先要求，浏览器、视觉、移动端、Provider 和真实学生验收全部记录为 deferred，本批不以其阻塞实现发布。
