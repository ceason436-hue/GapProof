...
current_design_stage: "学生端今日页 F0 已在独立 Worktree 完成视觉与技术验收、尚未合并/接入真实 API；后端已支持 D+1 调度与到期激活，复测作答评分和后续页面待推进"
...
`[PROTOTYPE]` 已在 Stitch 中完成并选定学生端“今日”桌面高保真视觉基线；F0 已在隔离 Worktree 编码并通过构建、自动化与截图验收。当前实现仍使用 Mock 数据、尚未合并 `main` 或接入真实任务接口，因此不代表业务主闭环已经完成。
...
**已完成**：学生端“今日”页的桌面视觉结构、颜色角色、主要卡片层级和关键静态内容已确定；隔离 Worktree 中的 F0 已完成构建、自动化、1440×900/1366×768 截图及滚动固定行为验收。

**尚未完成**：F0 的分支保存、合并与真实 API 接入；其他学生 P0 页面、家长端页面、Logo 紧裁/SVG/Favicon 与最终品牌规范、完整交互原型。后端已能生成最小干预、调度并到期激活 D+1，但 D+1 作答/评分及其页面仍未实现。

**下一步**：先将已验收 F0 保存到 `codex/` 前端分支并基于最新 `main` 复核后合并；随后用共享 contracts 接入真实今日任务接口，再按“开始/添加材料 → 识别确认 → 找原因 → 确认小题 → 7 日计划 → 学习室 → 报告”的主闭环设计其余学生页面。
...
4. **稍后继续与下次检查**：右栏展示最多两项稍后继续内容和一张下次检查卡。下次检查明确显示日期、要复习的内容、形式、预计时长、`明天再试` 按钮及“完成后会更新学习记录”的说明。后端已支持 D+1 任务到期 `scheduled → ready`，但 F0 尚未接入真实接口，D+1 作答与评分也尚未实现。
...
首页可使用低对比度书本、确认框、路径或箭头作为卡片右下角装饰，但图形必须与卡片内容相关、完整显示且不替代文字；不得留下裁切图案、孤立黑线或无法解释的伪进度条。
...
学习任务提交使用 `idle → submitting → completed`，旁路为 `conflict`、`invalid`、`failed`。提交前必须确保 `completedStepIds` 恰好覆盖全部步骤；提交中禁用重复点击但保留本地勾选。成功后展示“本次练习已完成，明天会有一次短检查”，并使用服务端 `scheduledRetest.scheduledFor` 显示时间；不得表述为“已经修复”或“已经掌握”。D+1 任务为 `scheduled` 时 CTA 不可提前作答，只显示计划时间；服务端变为 `ready` 后才可显示“开始检查”。当前 D+1 作答与评分尚未实现，因此即使任务已 `ready` 也必须标明“检查功能即将开放”，不能由前端模拟完成或通过。Demo 虚拟时钟只供演示控制，不由普通学生页面自行调用。
...
- `[PROTOTYPE]` 学生端“今日”页桌面视觉基线已选定，规范见 5.6.1；前端 F0 已在独立 Worktree 完成多状态页面、构建、自动化、截图与固定壳层滚动验收，但仍为 Mock、尚未合并和接入真实 API。后端已有 Case、识别确认、竞争性错因、确认小题、最小干预、今日任务、完成/D+1 调度及到期激活能力，但尚无真实端到端页面接入证据。
