...
current_design_stage: "学生端今日页桌面视觉基线已选定；后端已生成竞争性错因并进入等待确认小题但尚未接入页面，其余页面与前端实现待推进"
...
`[PROTOTYPE]` 已在 Stitch 中完成并选定学生端“今日”桌面高保真视觉基线。它用于后续页面设计和前端截图回归，不代表该页面已经编码、可点击，或已接入真实任务数据。
...
**已完成**：学生端“今日”页的桌面视觉结构、颜色角色、主要卡片层级和关键静态内容已确定。

**尚未完成**：其他学生 P0 页面、家长端页面、Logo 紧裁/SVG/Favicon 与最终品牌规范、交互原型、前端组件与真实数据接入。后端虽已生成两条竞争性错因和一条确认小题并进入等待作答，但尚未形成任何页面实现或小题提交证据。

**下一步**：先将本页拆为可实现组件并保留截图基线；再按“开始/添加材料 → 识别确认 → 找原因 → 确认小题 → 7 日计划 → 学习室 → 报告”的主闭环设计其余学生页面，最后设计家长概览与比赛评委最小演示页。
...
- 不采集学校、班级、身份证明等非必要信息。
- 不以成绩区间给学生贴等级标签。
- 教材可预置为 ISBN `978-7-5720-3630-9`；版权页补齐前，版次/印次显示“待核对”，不得自动填入未经证实的出版年月。
- 教材字段必须支持“已确认”“待核对”“发现冲突”三种状态；冲突时展示书名/ISBN 差异并引导上传版权页，不由系统静默选择版本。
...
2. **重点任务卡**：黑色/深色卡片，包含任务标题、推荐原因、三枚紧凑步骤胶囊 `看一个例子 → 做一道确认小题 → 换一道新题再试`、右上角青柠绿“预计 10 分钟”胶囊，以及青柠绿、深色文字的 `开始今天的任务` 主按钮。卡片右下可放低对比度、完整可见的深灰书本轮廓；卡内不使用蓝色主强调。
...
4. **稍后继续与下次检查**：右栏展示最多两项稍后继续内容和一张下次检查卡。下次检查明确显示日期、要复习的内容、形式、预计时长、`明天再试` 按钮及“完成后会更新学习记录”的说明。此处是计划展示，真实定时调度仍待实现。
...
- `[PROTOTYPE]` 学生端“今日”页桌面视觉基线已选定，规范见 5.6.1；后端已有 Case、识别确认、竞争性错因和确认小题读取接口，但相关页面尚无前端实现、可点击原型或接口接入证据。
