...
current_design_stage: "学生端今日页 F1b 已完成共享 Today 契约的显式只读 API 模式、默认入口仍为 Mock；后端已支持 D+1 客观复测评分，前端作答与 D7/报告页面待推进"
...
`[PROTOTYPE]` 已在 Stitch 中完成并选定学生端“今日”桌面高保真视觉基线；F0 Mock 与 F1b 显式只读 API 模式均已通过构建、自动化与截图验收后合并 `main`。默认入口仍为 Mock，API 模式没有写入能力，因此不代表业务主闭环已经完成。
...
**已完成**：学生端“今日”页的桌面视觉结构、颜色角色、主要卡片层级和关键静态内容已确定；F0 Mock 与 F1b 显式只读 API 模式已完成构建、自动化、1440×900/1366×768 截图及滚动固定行为验收。F1b 使用服务端学生时区和 `currentTaskId`，并对 guided/D1/D7 作真实只读区分。

**尚未完成**：默认入口从 Mock 切换到正式 API、D1 作答交互、首页足迹/周目标/待确认/最近进展的真实投影；其他学生 P0 页面、家长端页面、Logo 紧裁/SVG/Favicon 与最终品牌规范、完整交互原型。后端已能完成 D1 客观评分，但 D7 作答、报告及其页面仍未实现。

**下一步**：在 F1b 只读基线上使用共享 attempts contracts 接入 D1 作答、权威 `expectedVersion`、`Idempotency-Key` 和错误/重试状态；D7 在后端契约实现前继续只读。其余页面按“开始/添加材料 → 识别确认 → 找原因 → 确认小题 → 7 日计划 → 学习室 → 报告”的主闭环推进。
...
3. **本周学习足迹**：使用 7 天方块矩阵和一句简短说明（如“本周已坚持 2 天”）。只用蓝色深浅表示完成度：深蓝 `#0036FF`、中蓝 `#5577FF`、浅蓝 `#B8C7FF`、未开始 `#EEF0F2`；今天用蓝色描边/文字标识，不在日期或方块中央额外加点，不混入绿色。
4. **稍后继续与下次检查**：右栏展示最多两项稍后继续内容和一张下次检查卡。下次检查明确显示日期、要复习的内容、形式、预计时长、`明天再试` 按钮及“完成后会更新学习记录”的说明。后端已支持 D+1 任务到期与客观评分；F1b 只读 API 模式按学生时区展示 D1/D7 的计划与截止时间，但尚不提供作答 CTA，首页其他统计投影无真实数据时保持不可用态。
5. **今日概览**：主卡下方为白色外层容器，内含并列的 `等你确认` 与 `最近进展` 两张同尺寸青柠绿卡。两卡均为 `#B5F800` 绿底黑字、16px 圆角、相同的轻微立体感；“等你确认”保留原色图标和黑底白字 `去确认` 按钮，“最近进展”不强制 CTA。每张卡右下只可放与内容相关、低透明度、完整显示的图形：确认卡可用对勾/确认框，进展卡可用路径/箭头/阶梯；禁止裁切图形、无含义黑线或伪进度条。
...
学习任务提交使用 `idle → submitting → completed`，旁路为 `conflict`、`invalid`、`failed`。提交前必须确保 `completedStepIds` 恰好覆盖全部步骤；提交中禁用重复点击但保留本地勾选。成功后展示“本次练习已完成，明天会有一次短检查”，并使用服务端 `scheduledRetest.scheduledFor` 按学生时区显示时间；不得表述为“已经修复”或“已经掌握”。D+1 任务为 `scheduled` 时 CTA 不可提前作答，只显示计划时间；服务端变为 `ready` 后，只有 F1c 使用共享 attempts contracts、权威 `expectedVersion` 与同一幂等键重试时才可开放作答。F1b 即使看到 ready D1 也保持“作答接入下一阶段”；D7 在其 attempts 实现前始终只读。Demo 虚拟时钟只供演示控制，不由普通学生页面自行调用。
...
- `[PROTOTYPE]` 学生端“今日”页桌面视觉基线已选定，规范见 5.6.1；前端 F0 Mock 与 F1b 显式只读 API 模式已完成多状态页面、构建、自动化、API/Mock 双视口截图与固定壳层滚动验收并合并 `main`。F1b 已消费服务端学生时区/currentTaskId 和 guided/D1/D7 任务状态，但默认入口仍为 Mock、没有 POST；后端已有 D1 客观评分与失败异步重排骨架，尚无真实端到端页面写入证据。
