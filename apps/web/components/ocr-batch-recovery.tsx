...
const copy = {
  ...
  retry: { title: "这份材料需要重新识别", detail: "上次识别没有完成。重新确认后，可以发起一次真实重试。", action: "查看并重试" },
} as const;
  ...
  return <section className={`ocr-recovery ${compact ? "compact" : ""}`} aria-labelledby="ocr-recovery-title" data-ocr-recovery-count={batches.length}>
    ...
    <div className="ocr-recovery-list">{batches.map(batch => {
      ...
      return <article key={batch.batchId} data-resume-kind={batch.resumeKind}>
        <div><strong>{item.title}</strong><p>{item.detail}</p><small>{batch.pageCount} 张图片</small></div>
        <Link className="secondary-button" href={href(batch)}>{item.action}</Link>
      </article>;
    })}</div>
  </section>;
}
