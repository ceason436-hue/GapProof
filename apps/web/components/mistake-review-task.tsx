...
export function MistakeReviewTask({ task }: { task: MistakeReviewTaskView }) {
  ...
  const resultUnknown = state === "error" && errorCode === "NETWORK_UNKNOWN";
  return <div className="guided-task-panel" data-mistake-review-state={state}>
    ...
    {state === "error" ? <p className="guided-task-feedback error" role="alert">{errorCode === "INVALID_INPUT" ? "请先写下你的想法，再完成这次复习。" : errorCode === "INVALID_TASK_STATE" ? "这项复习已经更新，请回到错题本查看。" : resultUnknown ? "暂时无法确认是否记录成功。为避免重复提交，请重新打开错题本读取最新状态。" : "暂时无法记录这次复习，请稍后再试。"}</p> : null}
    {resultUnknown ? <div className="guided-task-result-actions"><Link className="primary-blue" href="/student/mistakes">返回错题本</Link><Link className="secondary-button" href="/student/today?source=api">查看今日安排</Link></div> : <button className="guided-task-submit" type="button" onClick={() => { void submit(); }} disabled={state === "submitting" || responseText.trim().length === 0}>{state === "submitting" ? "正在记录" : "完成这次复习"}</button>}
  </div>;
}
