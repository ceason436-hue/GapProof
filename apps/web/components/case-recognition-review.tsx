  ...
  return <AppShell actionHref="/student/today" actionLabel="返回今日">
    <section className="case-review-page" data-review-state={state} aria-labelledby="case-review-title">
      ...
      {extraction && ["ready", "confirming", "confirm_conflict", "confirm_error", "confirm_unknown"].includes(state)
        ...
        : null}
      {state === "confirmed"
        ? <section className="case-review-state" data-review-confirmed><h2>识别内容已由你确认</h2><p>下一步会准备找原因的确认小题，不代表识别正确或产生学习结论。</p>{realExtraction ? <div className="case-review-feedback" data-original-image-state={originalImages}><p>{originalImages === "deleted" ? "原图已删除；已确认的文字内容仍会保留。" : originalImages === "unknown" ? "原图删除结果暂时无法确认。请先读取最新状态，确认前不会再次删除。" : originalImages === "error" ? "原图暂时未能删除，可以再次尝试。" : "原图将在 24 小时后自动删除，也可以现在删除。已确认的文字内容会继续保留。"}</p>{originalImages === "unknown" ? <button type="button" className="secondary-button" disabled={recoveryBusy} onClick={() => void recoverOriginalImagesDeletion()}>{recoveryBusy ? "正在读取状态" : "读取最新状态"}</button> : originalImages !== "deleted" ? <button type="button" className="secondary-button" disabled={originalImages === "deleting"} onClick={() => void deleteOriginalImages()}>{originalImages === "deleting" ? "正在删除原图" : originalImages === "error" ? "重新删除原图" : "现在删除原图"}</button> : null}</div> : null}<button type="button" className="primary-blue" onClick={() => void startHypotheses()}>开始找原因</button></section>
        : null}
