...
| PUSH-075 | 2026-08-16 | `main` | `pushed` | `feat: page mistake book queries` | `GET /question-archive` 新增受限 query/filter/cursor/limit 查询契约，返回 `totalCount`、`matchedCount`、最多 20 道记录和下一页不透明游标；Web 首屏服务端读取，查询/筛选远程刷新，继续加载去重追加，失败保留当前结果 | 按比赛功能落地优先要求未运行测试、集中审查、Provider smoke、浏览器/视觉或真实学生验收，全部 deferred；仓储仍先重建当前学生档案再分页，游标不含答案或内部 Case ID，读取不写任务状态，不证明真实学习效果 |
...
- 新增材料改名请求契约、材料档案批次版本及受设备所有权守卫的改名命令；事务锁定批次与 Case，以 expectedVersion 和请求指纹保护幂等重放，同时更新时间与批次版本。
- Web 保持材料列表服务端读取，仅将单卡改名表单设为客户端边界；成功刷新权威投影，冲突或网络未知不自动覆盖。无数据库迁移；测试、构建与审查继续 deferred。

### v0.3.85 — 2026-08-16

- 新增 `StudentMaterialArchiveItem/View` 契约及 `GET /v1/students/:studentId/materials`；中央设备所有权守卫限制 path student，仓储过滤 synthetic、simulation 和已删除 Case，聚合页数并返回最近 100 项与独立总数。
- Web 服务端页面复用设备会话 Cookie 读取材料档案，根据批次与 Case 权威状态生成后续入口；不在前端推断 OCR 正确、诊断结论、个性化或学习效果。无数据库迁移；测试、构建与审查继续 deferred。

### v0.3.84 — 2026-08-16

- `CreateRealOcrBatchRequest`、`RealOcrBatchView` 与 `RecoverableOcrBatchView` 增加 1–80 字无控制字符标题；持久化复用 `cases.title`，未给 `ocr_batches` 增加重复列或 migration。
- `findOcrBatch` 联接 Case 返回标题；创建幂等重放校验学生与规范化标题。设备恢复投影读取 Case 标题，并对旧值做非空及 80 字公开边界归一。
- Web 为新批次要求材料名称，创建意图形成后锁定输入；未知结果沿用同一名称和幂等键。恢复卡展示材料名称，后续事实投影自然复用 Case 标题。
- 按比赛功能落地优先要求，本批测试、审查及完整验收继续 deferred。

### v0.3.83 — 2026-08-16

- `QuestionArchiveQuery` 限制查询文本 200 字、游标 256 字、每页 1–50 道；默认筛选全部、每页 20 道。`QuestionArchiveView` 新增整本总数、命中总数和可空下一页游标。
- API 在学生/租户范围内重建已确认真实材料档案，按确认时间和不透明引用稳定排序后执行搜索、任务状态筛选及游标切页；无效游标返回受控 400。
- Web Client Component 对查询使用 250ms 延迟与 AbortController，继续加载只执行 GET 并按 `entryRef` 去重；失败不清空当前题目。当前不是数据库底层 keyset 扫描，后续规模需要时再下推仓储查询。
- 按比赛功能落地优先要求，本批测试、审查及完整验收继续 deferred。
